...
class QuranApi {
  ...
  static String normalizeArabic(String input) {
    ...
    //    Unicode: U+064B..U+065F, U+0670, U+06D6..U+06ED
    text = text.replaceAll(
      RegExp(r'[\u064B-\u065F\u0670\u06D6-\u06ED]'),
      '',
    );

    // 2) إزالة التطويل (Tatweel) — U+0640
    text = text.replaceAll('\u0640', '');

    // 3) إزالة الهمزات والألف بأشكالها → أ
    text = text
        .replaceAll('\u0622', '\u0627') // آ → ا
        .replaceAll('\u0623', '\u0627') // أ → ا
        .replaceAll('\u0625', '\u0627') // إ → ا
    ...
    // 5) توحيد التاء المربوطة والهاء
    text = text.replaceAll('\u0629', '\u0647'); // ة → ه

    // 6) إزالة الأقواس والعلامات
  ...
  /// مقارنة بحث: هل يحتوي النص على الاستعلام؟ (بدون تشكيل)
  ...
  static Future<Map<String, dynamic>> fetchSurahWithAudio(
    ...
    final url =
        Uri.parse('$_baseUrl/surah/$surahNumber/$reciterIdentifier');
    final response = await http.get(url).timeout(const Duration(seconds: 30));
